  ...
  adicionarPedido = async () => {
    ...
    try {
      ...
      if (data?.erro) {
        ...
        const restante = data.quantidade;
        alert(`Quantidade insuficiente. Restam apenas ${String(restante ?? 0)}`);
        return;
      }
      ...
      const quantidadeR = Number(data?.quantidade ?? 0);
      const novaQ = quantidadeR - quantidade;
      if (!isNaN(novaQ) && isFinite(novaQ) && novaQ >= 0) {
        if (novaQ === 0) {
    ...
    try {
      ...
      if (comand && pedido && quantidade) {
        ...
        try {
          ...
          data = { erro: true, mensagem: 'Falha ao checar estoque.' };
        }
        ...
        if (data?.erro) {
          ...
          const restante = data?.quantidade;
          alert(`Quantidade estoque insuficiente. Restam apenas ${String(restante ?? 0)}`);
          return;
        }
